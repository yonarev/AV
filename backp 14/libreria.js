...
function genIdTime(){
  ...
  const mesHoy = ("0" + (currentDate.getMonth() + 1)).slice(-2); // Agrega un 0 delante del mes si es menor que 10
  const diaDeHoy = ("0" + currentDate.getDate()).slice(-2); // Agrega un 0 delante del día si es menor que 10
  const fechaHoy = anioHoy + mesHoy + diaDeHoy; // entrega fechaHoy global
  //fecha actual exacta
  const fechaIng = `${diaDeHoy}/${mesHoy}/${anioHoy}` // en variable global fechaIng
  // let id=fecha+hora
